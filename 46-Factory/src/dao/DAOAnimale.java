...
public class DAOAnimale extends DAO 
{
	...
	{
		return super.update("INSERT INTO animali (nome, specie, dob, genere, peso, vaccinato) VALUES " +
							"(\"" + ((Animale)e).getNome()   	+ "\", "  +
							"\""  + ((Animale) e).getSpecie() 	+ "\","  +
							"\""  + ((Animale)e).getDob()   	+ "\","   +
							"\""  + ((Animale)e).getGenere() 	+ "\"," +
	...
	{
		return super.read("SELECT * FROM Animali");
	}
	...
	public List<Animale> list()
	{
		...
		for(Map<String, String> riga : righe) 
		{
			...
			// Tramite il metodo create sfrutto la mappa e rimepio la proprieta' dell'oggetto
			a.create(riga);
			
			
			// Aggiungo l'oggetto castato(perche' formalmente e' di tipo IFactory) alla lista finale
			ris.add((Animale)a);
		}
